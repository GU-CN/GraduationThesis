...
\addcontentsline{toc}{chapter}{Appendix II.\ Hardware Specifications and Parts Records}
...
Each gripper assembly uses two fingers of the same geometry. The left gripper has two TPU~64D fingers, while the right gripper has two TPU~95A fingers, giving four fingers in the dual-arm system. One finger has an overall bounding box of \(123.36\times25.80\times40.00~\mathrm{mm}\), corresponding to \(L/T\approx4.78\) and \(L/H\approx3.08\). It is screw mounted, with a reinforced attachment region and a tapered ribbed body.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{p{0.28\textwidth}X}
    ...
    \midrule
    Material & Left gripper: TPU, Shore hardness 64D; right gripper: TPU, Shore hardness 95A \\
    Overall size & \(123.36\times25.80\times40.00~\mathrm{mm}\) per finger \\
    Quantity & Two TPU~64D fingers on the left gripper and two TPU~95A fingers on the right gripper \\
...
\begin{figure}[htbp]
  ...
  \caption{Orthographic views of the common compliant-finger geometry used for the TPU~64D left-gripper pair and TPU~95A right-gripper pair.}
...
Table~\ref{tab:app-parts-components} preserves the complete system composition. Chapter~4 contains only a system-level summary so that component model numbers and procurement-oriented details do not interrupt the engineering argument.
...
\begin{longtable}{
  ...
  4 & Compliant finger & TPU~64D (left pair) and TPU~95A (right pair), \(123.36\times25.80\times40.00~\mathrm{mm}\), tapered and ribbed & 4 & Object contact with passive geometric tolerance \\
