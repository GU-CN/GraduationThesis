...
\subsection{Engineering Change Notice from DR3 to the final prototype}

Table~\ref{tab:ch4-ecn} records the material changes between the DR3 baseline
and the implemented prototype. The purpose of this Engineering Change Notice
(ECN) is not to restate the project history. It identifies which interfaces
changed, why the change was accepted, which evidence verifies the final state,
and which limitation remains after implementation.
...
\begin{longtable}{
  ...
  \caption{Engineering Change Notice from the DR3 baseline to the final prototype}
  \label{tab:ch4-ecn}\\
  \toprule
  \textbf{ECN} & \textbf{DR3 baseline} & \textbf{Final implemented state} &
  \textbf{Reason and affected interface} & \textbf{Verification and residual risk} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{ECN} & \textbf{DR3 baseline} & \textbf{Final implemented state} &
  ...
  \endhead
  01 &
  Task-specific ACT checkpoints selected by the operator. &
  One language-conditioned LeRobot $\pi_{0.5}$ checkpoint is shared by all three tasks. &
  The change removes manual policy-file switching and makes the instruction, three images, and bilateral state one common policy input. &
  The 65,000-step checkpoint completed all three physical tasks. No matched ACT trial set was run, so comparative superiority is not claimed. \\
  02 &
  Single-arm and staged validation evidence was used while the dual-arm route remained the target architecture. &
  Two FR3 arms and two grippers are represented by one 16-dimensional bilateral state and one 16-dimensional absolute action. &
  The data, model adapter, and robot adapter now use the same \(7+1+7+1\) ordering. &
  Dataset metadata, checkpoint configuration, dry runs, and physical trials verify the interface. Exact base poses and swept-volume geometry remain documentation gaps. \\
  03 &
  Earlier policy work used task-specific datasets and did not establish a common multi-task sampler. &
  \texttt{dual\_ring}, \texttt{dual\_socket}, and \texttt{dual\_ram} are trained jointly with equal task probability. &
  Equal task probability prevents the ring dataset, which has the most frames, from dominating task selection during training. &
  The dataset manifest and sampler configuration verify the three-task scope. The compatible \texttt{dual\_socket2} directory was not included. \\
  04 &
  Manual checkpoint identity was coupled to the selected task. &
  Checkpoint selection uses the unweighted macro flow-matching loss of the three tasks; step 65,000 is selected. &
  A task-balanced offline rule is used before physical testing, avoiding selection from the final 75 task outcomes. &
  Step 65,000 has macro loss 0.036552. Offline loss remains a model-selection quantity, not a physical success metric. \\
  05 &
  ACT predicted a 32-step chunk and the earlier executor operated under a different policy-specific temporal interface. &
  $\pi_{0.5}$ predicts 50 actions and the executor consumes the first 10 at the configured 5~Hz cadence before replanning. &
  The longer prediction horizon supplies temporal context while the shorter executed segment limits open-loop motion. &
  Shape and queue behavior are verified in dry runs and deployment. End-to-end query latency was not measured. \\
  06 &
  DR3 listed planned ES1--ES6 validation activities. &
  The final report contains completed ES1--ES6 results and 75 counted end-to-end task trials. &
  The validation record now separates planning, task execution, detection, pose error, failure detection, and recovery rather than treating one aggregate rate as complete validation. &
  Chapter~\ref{chap:validation} reports denominators and pass/fail judgments. The ES4 raw error series is unavailable and is not reconstructed. \\
  07 &
  The current controller and source state were described primarily by a repository branch and intended scripts. &
  The report records the actual local training files, environment versions, file hashes, checkpoint assets, and uncommitted-source risk. &
  Software, model weights, normalization assets, and run configuration are treated as virtual implementation materials. &
  The final checkpoint is portable, but the Git commit alone cannot reproduce the training run because relevant files were untracked or locally modified. \\
  08 &
  The proof-of-concept budget emphasized zero incremental cash expenditure. &
  The final budget separates zero additional cash cost from in-kind robots, cameras, compute, storage, fabrication, software, and labour. &
  The distinction prevents a laboratory system from being represented as having zero economic value. &
  The resource categories are recorded; replacement values, print time, energy, and labour were not supplied. \\
  \bottomrule
\end{longtable}
\endgroup

The ECN shows that the central architectural change is concentrated in the
learning and validation layers. The two-arm mechanical platform, sensing
layout, demonstration interface, compliant fingers, and deterministic
robot-side protection remain continuous with DR3. This continuity makes the
physical workcell a controlled basis for evaluating the new policy route, but
it does not create a controlled ACT-versus-\(\pi_{0.5}\) comparison because the
earlier and final controllers were not tested under one matched protocol.
...
\subsection{VLA input--output formulation and notation}
...
The complete joint ranges, mounting specification, environmental limits, controller interfaces, and dimension drawings are reference data rather than design arguments. They are collected in Appendix~\ref{chap:hardware-records}, Table~\ref{tab:app-fr3-vendor-parameters}, and Figures~\ref{fig:app-fr3-overview}--\ref{fig:app-fr3-configuration}. The practical task workspace is deliberately smaller than those hardware bounds so that action chunks do not intentionally approach joint limits.

Figure~\ref{fig:ch4-fr3-workspace} is retained in the main analysis because it directly supports placement of the shared task region. The 855~mm maximum reach is not treated as an invitation to place objects at the boundary: motion near maximum extension reduces available orientation freedom and can bring the arm closer to singular configurations.
...
\subsection{GELLO channel architecture and leader--follower mapping}
...
\subsection{Dataset inclusion boundary and language-prompt contract}

The final checkpoint is a three-dataset model. A fourth local directory,
\texttt{dual\_socket2}, contains the same LeRobot Dataset v3.0 feature schema
and 80 episodes, but it is not enumerated by the training program used for the
reported run. It therefore contributes no frame to the normalizer, sampler,
optimizer updates, validation loss, checkpoint selection, or physical result.
Schema compatibility is not evidence of use. Table~\ref{tab:ch4-dataset-scope}
records this boundary explicitly.

\begin{table}[htbp]
  \centering
  \caption{Dataset and instruction scope of the deployed $\pi_{0.5}$ checkpoint}
  \label{tab:ch4-dataset-scope}
  \footnotesize
  \begin{tabularx}{\textwidth}{
    ...
    X}
    \toprule
    \textbf{Directory} & \textbf{Episodes} & \textbf{Used} &
    \textbf{Training instruction or exclusion} \\
    \midrule
    \texttt{dual\_ring} & 80 & Yes &
    First, push the can to the middle, then pick up the black ring with the handle, put it over the can, and set it down. \\
    \texttt{dual\_socket} & 80 & Yes &
    Push the box with the power socket to the middle, then pick up the plug and insert it into the socket. \\
    \texttt{dual\_ram} & 80 & Yes &
    Pull the computer motherboard closer, then remove the memory module and place it in the yellow tray. \\
    \texttt{dual\_socket2} & 80 & No &
    Compatible local data directory, but absent from the task-prompt map and from the reported training, statistics, validation, and checkpoint. \\
    \bottomrule
  \end{tabularx}
\end{table}

The raw \texttt{tasks.parquet} metadata in each used directory contains the
generic source task \texttt{franka\_gello\_teleop}. During loading,
\texttt{PromptDataset} replaces that generic value with the task-specific
instruction in Table~\ref{tab:ch4-dataset-scope}. The prompt is consequently
part of the executable training configuration rather than an informal caption
added after data collection. Deployment uses the same instruction wording.

If \texttt{dual\_socket2} is added in a later experiment, its statistical and
sampling role must be decided before training. Treating four directories as
four equal sampler groups would give the two socket directories a combined
50\% task probability. Treating them as two sources of one semantic socket
task would instead require a hierarchical sampler so that ring, socket, and
RAM remain approximately one third each. Either choice also requires new
normalization statistics, validation reporting, checkpoint selection, and
physical evaluation; the present 65,000-step result cannot be re-labelled as a
four-dataset experiment.

\subsection{Training statistics and task-balanced sampling}
...
\begin{center}
\small\texttt{dc06dba9d6220ab99c94e83ec7f0d45a35b44bed}.
\end{center}
...
\subsection{Model input construction and state--language representation}
...
experiment does not isolate their individual effects. Because each visually
distinct scene was paired with its expected instruction, the evidence supports
a language-conditioned unified interface rather than a claim that language
alone determines the selected behavior.
...
\subsection{Adaptation scope and trainable parameters}

The final run uses action-expert-only adaptation. The entire PaliGemma
vision-language component, including the SigLIP visual encoder, remains frozen.
Trainable names are restricted to the Gemma action expert and the action/time
projection modules. No LoRA or other PEFT adapter is used in this LeRobot
experiment.
...
the base representation. A separate OpenPI/JAX socket experiment contains
LoRA parameters and an eight-dimensional single-arm state/action record, but
it is not the source of the deployed dual-arm checkpoint and is excluded from
the final design claim.
...
\subsection{Optimizer schedule, runtime, and checkpoint package}
...
Saving complete model and optimizer states supports continuation and auditing,
but it has a substantial storage cost. It also does not, by itself, make the
experiment fully reproducible: model artifacts preserve the trained state,
whereas source, environment, data identity, and base-weight provenance
...
Table~\ref{tab:ch4-budget} separates the reported incremental cash cost from the laboratory equipment, compute, and fabrication resources supplied in kind. This distinction avoids presenting an in-kind research workcell as a zero-value system.
...
\subsection{Software, environment, and checkpoint provenance}

The project records Git
\texttt{HEAD=dc06dba9d6220ab99c94e83ec7f0d45a35b44bed}, but that identifier is
only the base repository state. At inspection time, the index contained
approximately 705 staged deletions, the current project files appeared again
as untracked files, and the final $\pi_{0.5}$ training program was itself
untracked. The commit therefore cannot be cited as a complete source snapshot
for the 100,000-step run.

To make the inspected implementation identifiable even without a clean commit,
Table~\ref{tab:ch4-source-hashes} records hashes for the training path that
produced the final experiment.

\begingroup
\footnotesize
\begin{longtable}{
  >{\raggedright\arraybackslash}p{0.40\textwidth}
  >{\ttfamily\raggedright\arraybackslash}p{0.52\textwidth}}
  \caption{SHA-256 identifiers for key $\pi_{0.5}$ implementation files}
  \label{tab:ch4-source-hashes}\\
  \toprule
  \textbf{Project-relative file} & \textbf{SHA-256} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Project-relative file} & \textbf{SHA-256} \\
  \midrule
  \endhead
  \texttt{train/train\_lerobot\_pi05\_multitask.py} &
  83d3ddbb4319dc225701cb8f236cbfea77b6d7d61ebfc7d0b994244ef092b8b8 \\
  \texttt{scripts/train\_dual\_pi05\_multitask.sh} &
  05c717449929362d84f1b1068c97fd02939bc7a71616fc1329f0c7daa114799c \\
  \texttt{scripts/train\_dual\_pi05\_multitask.sbatch} &
  a6d862671837ff11c1d2c50f9f76549eab2afbfb606e988834e6f5613462550d \\
  \texttt{configuration\_pi05.py} &
  c66dfddc27557dd89a1cabd1354f041923bfcb541d348185a9a1af7fab643a8b \\
  \texttt{modeling\_pi05.py} &
  263a2c1e40e5a1cf0ab0808f149a1a383b337c0ace4bddff15e26119449648f9 \\
  \texttt{processor\_pi05.py} &
  ca9cca8d54a681e13072b3acfb557f7b8e77c37ac4452bdbc4f9e1773dd43dea \\
  \bottomrule
\end{longtable}
\endgroup

The observed training environment used Python~3.12.13,
LeRobot~0.5.2, PyTorch~2.10.0+\texttt{cu128}, and
Transformers~5.3.0. The supplied \texttt{environment.yml} does not lock this
complete stack. The converted base weights are present locally, but the
download identifier and conversion command were not retained with the run.
The final reproducibility claim is consequently partial: the checkpoint and
key files are identifiable and deployable, but recreating the same training
state from the repository alone is not yet guaranteed.

\subsection{Reproduction checklist for the final configuration}
...
\begin{enumerate}
  \item archive the exact hashed training, processor, executor, and deployment
        files under one clean commit;
  ...
  \item preserve the three used dataset versions, prompts, episode indices,
        normalization statistics, and the explicit \texttt{dual\_socket2}
        exclusion;
  \item retain the step-65,000 checkpoint, its tokenizer, processors,
        multi-task manifest, and training state;
