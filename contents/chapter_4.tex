...
Figure~\ref{fig:ch4-final-architecture} presents the final language-conditioned $\pi_{0.5}$ training and action-generation architecture. The observation input combines one overhead image and two wrist images as multi-view visual tokens. One of the three task instructions and the current 16-dimensional bimanual proprioceptive state are supplied through vision--language--state conditioning. During fine-tuning, the trainable LoRA adapters and robot action expert adapt the pretrained model to the recorded manipulation data. The conditioned action expert produces a continuous bimanual action chunk containing absolute joint targets and gripper widths for both arms. The 16 valid output dimensions are ordered as seven left-arm joints, the left-gripper width, seven right-arm joints, and the right-gripper width.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/overview_workflow_gai.png}
  \caption{Final language-conditioned $\pi_{0.5}$ architecture. Three RGB views, a task instruction, and 16-dimensional bimanual proprioception condition the unified model. Trainable LoRA adapters and the robot action expert generate continuous 16-dimensional action chunks containing absolute joint targets and gripper widths for the left and right arms.}
  \label{fig:ch4-final-architecture}
\end{figure}
...
The input side of Figure~\ref{fig:ch4-final-architecture} records the implemented observation contract: one overhead RGB view, two wrist RGB views, and the current 16-dimensional bimanual proprioceptive state. The physical workcell uses two FR3 arms, two custom grippers, and two GELLO leader assemblies for bilateral demonstration collection. Exact base spacing, camera height, camera pitch, and fixture coordinates were not included in the supplied implementation record; these values should be added to a future dimensioned workcell drawing rather than inferred from the model-architecture figure.
