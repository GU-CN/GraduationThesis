...
\end{align}
Thus, the three images and the measured robot state come from the same \texttt{current\_packet}. The supervised arm label is shifted forward by one packet so that the observation at \(t\) is paired with the next absolute joint target. The gripper label follows the existing same-packet convention and stores the absolute commanded opening width from packet \(t\).
...
LeRobot commit & \texttt{dc06dba9d6220ab99c94e83ec7f0d45a35b44bed}, with local modifications \\
