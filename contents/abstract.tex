...
\par
最终实机验证采用 65,000 步检查点，对每项任务进行了 25 次试验，共计 75 次。圆环任务、插头插入任务和内存条任务的成功率分别为 84\%、52\% 和 100\%，总体成功率为 78.7\%。独立的 25 次试验还得到规划成功率 92\%、元件检测率 96\%、失败检测率 92\% 和恢复成功率 92\%。对于 ES4，三个任务的夹具和终止区域均不允许在位置偏差达到或超过 20~mm 时完成任务，因此 59 个成功到达的终止状态均通过位姿估计误差阈值检查；该检查为二元判定，未记录连续的毫米误差值。在项目层面，六项工程指标的点估计或操作性测量均达到最终规定的阈值，但插头插入任务未能单独达到 70\% 的任务执行目标，表明接触密集型任务中的精细对准与插入仍是当前系统的主要限制。
\end{abstract}
