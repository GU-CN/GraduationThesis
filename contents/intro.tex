...
The engineering acceptance targets inherited from the design-specification stage are planning success above 90\%, task execution success above 70\%, component detection accuracy above 90\%, pose error below 20\,mm, failure-detection rate above 90\%, and recovery success above 70\%. These values define validation thresholds rather than performance claims. Because the implemented $\pi_{0.5}$ policy is an end-to-end controller without separate planning, detection, pose-estimation, or learned recovery modules, Chapter~\ref{chap:validation} distinguishes directly measurable end-to-end task outcomes from design-stage targets that are not independently observable in the final architecture.
...
\begin{enumerate}
  ...
  \item A scoped physical-validation framework defines task-specific terminal conditions, run-termination rules, and end-to-end evaluation procedures for the ring, socket, and memory-module tasks.
